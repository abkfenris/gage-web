...
def fetch_samples(sensor_ids, remote_type, remote_parameter):
    ...
    """
    logger.debug('Fetching a chunk of %s samples for sensors %s', remote_type, sensor_ids)
...
def chunk_sensor_ids(sensor_ids, remote_type, remote_parameter, delay):
    ...
    """
    logger.debug('Chunking %s sensors for tasks for sensors %s', remote_type, sensor_ids)
    try:
        x_range = xrange
    except NameError:
        x_range = range
